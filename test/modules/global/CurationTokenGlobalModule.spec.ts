...
import { DeployCtx, assetCuration, deployContracts, testToken, tokenGlobalModule, user, userAddress } from "../../setup.spec";
...
describe("Testing CurationTokenGlobalModule", () => {
  ...
  it("Should not create curation with token global module", async () => {
    ...
    await expect(curation.create("https://example.com", 0, [])).to.not.be.reverted;
    await expect(tokenGlobalModule.setCurationCreateFee(10)).to.not.be.reverted;
    await expect(curation.create("https://example.com", 0, [])).to.be.reverted;
  });
